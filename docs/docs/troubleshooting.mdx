...
Once all environments are migrated, you will need to update the permission level for these namespaces following the steps described in the [How to handle access restricted namespaces](/troubleshooting#handle-access-restricted-namespaces) section.
After this, you will be able to delete the old namespaces using the `delete` options from the conda-store UI `/conda-store/namespace/`.
