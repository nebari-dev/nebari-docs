...
this is a manual process that consists in copying over the yaml specifications for each environment to the `global` namespace and rebuilding them.
...
Once all environments are migrated, you will need to update the permission level for the old namespaces following the steps described in the [How to handle access restricted namespaces](/troubleshooting#handle-access-restricted-namespaces) section.
After this, you will be able to delete the `default` and `filesystem` namespaces using the `delete` option from the conda-store UI `/conda-store/namespace/`.
