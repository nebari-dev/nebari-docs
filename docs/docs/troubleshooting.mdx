...
After upgrading to `0.4.5`, the base namesfor the follow namespaces that are built in with Nebari were changed as follows:
- `default` -> `global`
- `filesystem` -> `nebari-git`

Due to this change, you will need to migrate the environments from the `default` into the new `global` namespace,
...
Unfortunately, this manual process needs to be performed for each one of the environments **individually** due to how conda-store handles assyncronous operations.

Once all environments are migrated, you can delete the `default` and `filesystem` namespaces following the steps described in the [How to handle access restricted namespaces](#how-to-handle-access-restricted-namespaces) section.
