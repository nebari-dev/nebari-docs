...
2. Build a GPU-enabled environment using `CONDA_OVERRIDE_CUDA`.
    1. At the time of writing this document the latest CUDA version was showing as `12.1`.
   You will need check PyTorch documentation to [select a supported version of CUDA from our support matrix](https://pytorch.org/get-started/locally/) to determine the latest override version for `CONDA_OVERRIDE_CUDA`.
