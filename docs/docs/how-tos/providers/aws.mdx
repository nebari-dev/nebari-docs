...
:::note[Cloudflare DNS]
To use `aws-config-with-dns.yaml`, [generate a Cloudflare API token](https://dash.cloudflare.com/profile/api-tokens) with `Zone:DNS:Edit` permission on the zone in `dns.cloudflare.zone_name`. See [DNS](/docs/how-tos/deploy#dns) in the deploy lifecycle guide for how to add it to `.env`.
:::
