...
:::important
If you configure dns with cloudflare, you must have the `CLOUDFLARE_API_TOKEN` environment variable set with a token that has permissions to edit DNS records for your domain. This is required for `nic` to create the necessary DNS record for your cluster's load balancer. 
Otherwise after `nic deploy` completes, it prints the load balancer's public IP address. Create an **A record** at your DNS provider pointing `<your-domain>` to that IP. The cluster will not be reachable over HTTPS until this record propagates.

See [DNS configuration](https://github.com/nebari-dev/nebari-infrastructure-core/blob/main/docs/design-doc/appendix/16-configuration-reference.md#dns-provider-configuration) for details.
:::
