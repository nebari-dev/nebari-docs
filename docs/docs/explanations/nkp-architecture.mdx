...
**Bottom up, here is what each layer is for:**

- **Cloud or bare-metal provider**: the physical infrastructure NKP runs on.
  This can be cloud VMs (AWS, GCP, Azure), dedicated physical servers from a
  bare-metal provider (Hetzner, Equinix Metal), or your own machine for local
  development. It supplies the network, identity, and durable storage every
  layer above depends on.
- **Managed Kubernetes cluster**: the engine that runs every container in NKP,
  including the foundational software and your applications. "Managed" means
  the cloud provider keeps the cluster healthy and up to date, so you don't
  have to. Use EKS on AWS, GKE on Google Cloud, AKS on Azure, or K3s locally
  for development.
- **Foundational software**: the shared services every Software Pack uses,
  like user login, traffic routing, and monitoring (collecting logs, metrics,
  and traces).
- **Nebari Operator**: an automation component that runs inside the cluster.
  Whenever a Software Pack arrives, the operator reads its declaration and
  hooks it into the foundational services (login, routing, monitoring)
  automatically.
- **Software Packs**: installable Capabilities like a chat assistant,
  document analyzer, or code review tool, authored by OpenTeams, customer
  teams, or the community. Each pack declares its requirements; the operator
  wires it into the platform.
- **Dynamic landing page**: the home page where users browse and open the
  Capabilities installed on the platform.
