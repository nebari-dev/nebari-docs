...
Nebari has a predefined (and configurable) storage limit for private and shared conda environments created via `conda-store`. A common pitfall is having the created environments fill up the available space, resulting in the following error in the conda-store UI when trying to [create new environments or builds](/tutorials/creating-new-environments):

> `CondaStore.storage_threshold` reached

To remove unused or unwanted environments and free up some space, you can follow the [Delete a conda-store build, environment or namespace](/troubleshooting#delete-a-conda-store-build-environment-or-namespace) steps.
