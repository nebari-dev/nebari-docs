...
If the nebari-dask environment does not include all the packages needed, here is a list to [create a new environment](/tutorials/creating-new-environments/):
...
This opens a familiar Keycloak authentication page, where you can [sign-in the same way you authenticated into Nebari](/tutorials/login-keycloak/).
...
<Tabs>
  <TabItem label="Browser-based diagnostic dashboard">
...
![Dask diagnostic UI - showing four parallel computation streams](/img/tutorials/dask_diagnostic_dashboard.png)
  ...
  <TabItem label="Dask JupyterLab extension (recommended)">
